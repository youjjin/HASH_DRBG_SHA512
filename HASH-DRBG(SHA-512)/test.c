...
void Hash_DRBG_SHA512_usePR_Test()
{
	...
	fp_req = fopen("HASH_DRBG(SHA512(use PR))_KAT.req", "r");
	fp_fax = fopen("HASH_DRBG(SHA512(use PR))_KAT.rsp", "w");
	...
	for(j = 0; j < 4; j++)
	{
		...
		for (i = 0; i < 15; i++)
		{
			...
			/****************HASH_DRBG***************/			
			Hash_DRBG_SHA512_yes(&state, Entropy, entropy_len, Entropy1, entropy1_len, Entropy2, entropy2_len, Nonce, nonce_len, Personalizationstring, personalization_len, Additional1, additionalinput1_len, Additional2, additionalinput2_len, pseudorandom_bits);
...
void Hash_DRBG_SHA512_noPR_Test()
{
	...
	fp_req = fopen("HASH_DRBG(SHA512(no PR))_KAT.req", "r");
	fp_fax = fopen("HASH_DRBG(SHA512(no PR))_KAT.rsp", "w");
	...
	for (j = 0; j < 4; j++)
	{
		...
		for (i = 0; i < 15; i++)
		{
			...
			/****************HASH_DRBG***************/
			Hash_DRBG_SHA512_no(&state, Entropy, entropy_len, Entropy1, entropy1_len, Entropy2, entropy2_len, Nonce, nonce_len, Personalizationstring, personalization_len, Additional1, additionalinput1_len, Additional2, additionalinput2_len, pseudorandom_bits);
